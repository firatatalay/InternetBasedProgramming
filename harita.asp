...
<body bgcolor="#EAEAEA" >
	...
	<br>
		...
		%>
			...
			width:455px;
			height: 455px;
			background-image:url(http://www.kgm.gov.tr/SiteCollectionImages/KGMimages/Haritalar/turistik.jpg);
			background-size:<%=zoom%>% <%=zoom%>%;
			background-position: <%=x%>px <%=y%>px;">
			</div>
			<center><table border="0">
				<tr>	<td><a href="harita.asp?x=<%=(x+100)%>&y=<%=(y+100)%>&zoom=<%=zoom%>"><input type="button" value="<^"  style="font-weight:bold;"></a></td>		<td colspan="2"><a href="harita.asp?x=<%=(x)%>&y=<%=(y+100)%>&zoom=<%=zoom%>"><input type="button" value="^"  style="font-weight:bold; width:160px;"></a></td>	<td><a href="harita.asp?x=<%=(x-100)%>&y=<%=(y+100)%>&zoom=<%=zoom%>"><input type="button" value="^>"  style="font-weight:bold;"></a></td>	</tr>
				<tr>	<td><a href="harita.asp?x=<%=(x+100)%>&y=<%=(y)%>&zoom=<%=zoom%>"><input type="button" value="<"  style="font-weight:bold;"></a></td>		<td><a href="harita.asp?x=<%=(x+150)%>&y=<%=(y+200)%>&zoom=<%=(zoom+50)%>"><input type="button" value="+"  style="font-weight:bold;width:80px"></a></td>	<td><a href="harita.asp?x=<%=(x-150)%>&y=<%=(y-200)%>&zoom=<%=(zoom-50)%>"><input type="button" value="-" style="font-weight:bold; width:80px"></a></td>				<td><a href="harita.asp?x=<%=(x-100)%>&y=<%=(y)%>&zoom=<%=zoom%>"><input type="button" value=">" class="butonlarim" style="font-weight:bold;"></a></td>	</tr>
				<tr>	<td><a href="harita.asp?x=<%=(x+100)%>&y=<%=(y-100)%>&zoom=<%=zoom%>"><input type="button" value="<v"  style="font-weight:bold;"></a></td>		<td colspan="2"><a href="harita.asp?x=<%=(x)%>&y=<%=(y-100)%>&zoom=<%=zoom%>"><input type="button" value="v"  style="font-weight:bold; width:160px;"></a></td>	<td><a href="harita.asp?x=<%=(x-100)%>&y=<%=(y-100)%>&zoom=<%=zoom%>"><input type="button" value="v>"  style="font-weight:bold;"></a></td>	</tr>
			</table>
			</center>
